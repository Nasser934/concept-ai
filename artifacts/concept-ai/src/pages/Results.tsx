...
import { saveReport, publishReport } from "@/lib/reports";
...
const Results = () => {
  ...
  const [resultData, setResultData] = useState<{ report: FeasibilityReport; inputs: ConceptInputs; slug?: string } | null>(null);
  const [savedReportId, setSavedReportId] = useState<string | null>(null);

  useEffect(() => {
    const load = () => {
      const raw = sessionStorage.getItem("__concept_ai_result__");
      if (raw) {
        try { setResultData(JSON.parse(raw)); } catch {}
      }
    };
    load();
    window.addEventListener("concept-ai-result", load);
    return () => window.removeEventListener("concept-ai-result", load);
  }, []);

  useEffect(() => {
    if (resultData?.slug) setShareSlug(resultData.slug);
  }, [resultData?.slug]);

  // Auto-save once on first load (private by default)
  useEffect(() => {
    if (!resultData?.report || !resultData?.inputs || resultData?.slug || shareSlug) return;
    let cancelled = false;
    saveReport(resultData.inputs, resultData.report)
      .then((d) => { if (!cancelled) { setShareSlug(d.slug); setSavedReportId(d.id); } })
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-background">
        <div className="text-center">
          ...
          <Button onClick={() => setLocation("/analyze")} className="mt-6">Start New Analysis</Button>
        </div>
      </div>
    );
  }
  ...
  const handleExportPptx = async () => {
    ...
    } catch (e: unknown) { toast.error(e instanceof Error ? e.message : "PPTX export failed", { id }); }
  ...
  const handleExportXlsx = async () => {
    ...
    } catch (e: unknown) { toast.error(e instanceof Error ? e.message : "XLSX export failed", { id }); }
    finally { setDownloading(false); }
  };
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      if (!id || !slug) {
        const d = await saveReport(inputs, report);
        id = d.id;
        slug = d.slug;
        setShareSlug(slug);
        setSavedReportId(id);
      ...
      // Publish it so the share link is accessible to anyone
    ...
    <div className="min-h-screen bg-[#eef2f7]">
      ...
      <nav className="sticky top-0 z-40 border-b border-border/60 bg-background/80 backdrop-blur-xl no-print">
        <div className="container mx-auto flex h-14 items-center justify-between px-6">
          <button onClick={() => setLocation("/")} className="flex items-center gap-2.5 text-foreground transition-colors hover:text-primary">
          ...
          <div className="flex items-center gap-2">
            <ThemeToggle />
            <Button variant="outline" size="sm" onClick={() => setLocation("/analyze")} className="h-8 gap-1.5 rounded-md border-border/70 bg-card/40 px-3 text-[13px] font-medium hover:bg-card">
