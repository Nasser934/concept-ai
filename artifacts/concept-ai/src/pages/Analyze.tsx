...
import { autofillBrief, completeField, analyzeConcept } from "@/lib/reports";
...
const Analyze = () => {
  ...
  const handleAutoFill = async () => {
    ...
    try {
      const data = await autofillBrief(brief);
  ...
  const handleCompleteField = async (field: EssayField) => {
    setCompleting(field);
    try {
      const data = await completeField(field, inputs[field] as string, inputs);
  ...
  const handleSubmit = async () => {
    ...
    setIsAnalyzing(true);
    try {
      const data = await analyzeConcept(inputs);
      setLocation("/results");
      // Store in sessionStorage so Results page can read it (wouter location.state workaround)
      sessionStorage.setItem("__concept_ai_result__", JSON.stringify({ report: data, inputs }));
      window.dispatchEvent(new Event("concept-ai-result"));
    } catch (e: any) {
  ...
  const EssayLabel = ({ field, children }: { field: EssayField; children: React.ReactNode }) => (
    <div className="flex items-center justify-between">
      ...
      <Button
        ...
        onClick={() => handleCompleteField(field)} disabled={completing === field}
      >
        {completing === field
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto max-w-2xl px-6 py-10">
        ...
        <div className="mt-10 flex items-center justify-between">
          <Button variant="outline" onClick={step === 0 ? () => setLocation("/") : prev} className="gap-2">
